function roi = ImROI(im, name, fsc, x, y , outfilename, plotopt)
%============================================================================
...
%
% parameters in   : image data, name, fsc (specifies whether image is shown
%                       full-screen)
...
if fsc
    ...
    imhandle = imshow(im,'InitialMagnification','fit');
else
    fighandle = figure;
    imhandle = imshow(im);
end
...
% First display an information window
message = sprintf(['The next ROI to mark is the ', upper(name), '. Please ', ...
    'draw a polygon around it by left-mouse-clicking. When you are ', ...
    'done, press any button. \nIf you cannot see the ', upper(name), ...
    ' in the video, please select "N.A.", otherwise "Proceed".']);

answer = questdlg(message, 'Information Window', 'Proceed','N.A.', 'Proceed');
...
if (nargin >= 5) 
   ...
   while ~ok
        ...
        % First display an information window
        message = sprintf(['Please confirm that you are happy with your ', ...
            'selection, otherwise, redo.']);

        answer = questdlg(message, 'Confirmation Window', 'Confirm','Redo', 'Confirm');
        ...
        switch answer
            ...
            case 'Redo'
                ok = 0;
                % Close the current image window (with the annotated line)
                % and open a new window showing a clean image
                close(fighandle);
                if fsc
                    fighandle = figure('units','normalized','outerposition',[0 0 1 1]);
                    imhandle = imshow(im,'InitialMagnification','fit')
